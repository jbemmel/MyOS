...
<!--
   Will generate component C++ glue code for MyOS3.x
...
<xsl:for-each select="requires/interfaceref">using <xsl:if test="@context"><xsl:value-of select='string:replaceAll(@context,"/","::")'/>::</xsl:if><xsl:value-of select="@name"/>;
</xsl:for-each>
...
: <xsl:value-of select="$superclass"/>( container, VERSION(<xsl:value-of select="version/@major"/>,<xsl:value-of select="version/@minor"/>), ID(), (IComponent*&amp;) instance )
{
}
