...
<xsl:result-document href="gen_src/{$cname}/{$c}_getput.cpp">
<xsl:variable name="xml"><xsl:call-template name="copyxml"/></xsl:variable>
#include "string.h"
#include "<xsl:value-of select='concat($cname,"/",$c)'/>.h"
...
MyOS::myos_result_t
<xsl:value-of select="$c"/>::get( const char* const call, MyOS::NVPAIR parameters[], IO::OStream&amp; output ) const {
...
MyOS::myos_result_t
<xsl:value-of select="$c"/>::put( const char* const call, MyOS::NVPAIR ps[], IO::IStream&amp; input ) {
<xsl:if test="$config/IDevice.XML">
...
public:
   ...
   inline <xsl:value-of select="$iname"/>( MyOS::Core::IComponent&amp; c, VERSION v)
   		: <xsl:value-of select="$super"/>(c,v<xsl:if test="@blocksize">,<xsl:value-of select="@blocksize"/></xsl:if>) {}
...
#include "<xsl:value-of select="@file"/>"</xsl:for-each>
	...
	virtual myos_result_t get( const char* const call, MyOS::NVPAIR parameters[], IO::OStream&amp; output ) const = 0;
	virtual myos_result_t put( const char* const call, MyOS::NVPAIR parameters[], IO::IStream&amp;  input ) = 0;
