...
<xsl:for-each select="requires/interfaceref">
using <xsl:if test="@context"><xsl:value-of select='string:replaceAll(@context,"/","::")'/>::</xsl:if><xsl:value-of select="@name"/>;
</xsl:for-each>
...
public:
	inline static const char* const ID() { return "<xsl:value-of select="@uuid"/>"; }
...
public: 
	inline static <xsl:value-of select="$name"/>&amp; getInstance() { return *instance; }
...
: IComponent( container, VERSION(<xsl:value-of select="version/@major"/>,<xsl:value-of select="version/@minor"/>), ID(), (IComponent*&amp;) instance )
<xsl:for-each select="provides/interfaceref">
...
myos_result_t <xsl:value-of select="$name"/>::initialize( Context::IContext&amp; context, NVPAIR params[] ) {
<xsl:if test="requires/interfaceref">myos_result_t r;</xsl:if>
<xsl:for-each select="requires/interfaceref">
r = context.lookup( myos_name_t( <xsl:value-of select='@name'/>::ID() ), (IInterface*&amp;) <xsl:value-of select="util:uncapitalize(@name)"/> );
...
myos_result_t <xsl:value-of select="$name"/>::deinitialize( Context::IContext&amp; context ) {
<xsl:for-each select="provides/interfaceref">
