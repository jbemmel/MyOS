...
// virtual 
physadr_t 
FATFileFragment::fetchNow() throw (MM::Exceptions::PagingException)
{
   ...
   class Handler : public IOCompletionHandler {
      ...
      Thread& reader;
public:
   inline Handler( FATFileFragment& frag ) 
      : fragment(frag), cur(0), result(0), reader( Thread::currentThread() ) {}
   
   inline physadr_t getResult() const { return result; }
   ...
   if (IThread::do_usSleep( 500000 )==false) {  // 500ms timeout, enough ?
      ...
      // return 0;
      throw MM::Exceptions::PagingException( /* timeout */ );
   }
}
