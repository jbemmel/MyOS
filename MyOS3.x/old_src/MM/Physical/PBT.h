...
namespace MM {
    ...
    public:
        ...
        inline static PBT& getCurrentPBT() {
      ...
      static inline bool isMapped( const PageDirectory& dir ) {
         return getCurrentPBT().entries[ getPDTIndex( (linadr_t) &dir) ].isMapped();               
      }
      
      static inline physadr_t getMapping( const PageDirectory& dir ) {
         return getCurrentPBT().entries[ getPDTIndex( (linadr_t) &dir) ].getMapping();        
      }
    };
