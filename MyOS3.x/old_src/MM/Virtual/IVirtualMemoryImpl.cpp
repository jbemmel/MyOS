...
IVirtualMemoryImpl::init( IContext& context, NVPAIR params[] ) /* INITSECTION */
{
   ...
   // NOTE: This frees UNCOMPRESSED pages, and also marks the stack page(s)
...
IVirtualMemoryImpl::allocPageNoThrow( physadr_t& mapping ) 
{
   ...
   if (startindex != kernelpages.E_FULL) {
      linadr_t a = (linadr_t) ((startindex << 12) + 0xFF800000);
      if ((mapping = PHYS.mapRegion( a, 0, E_PAGE_KERNEL ))) {      //< @todo add parameter for caller to set PTE bits
...
IVirtualMemoryImpl::allocateStack( size_t order, bool userMode ) throw (MM::OutOfMemoryException) 
{
   ...
   if (startindex != kernelpages.E_FULL) {
      ...
      if (PHYS.mapRegion( a, order, E_PAGE_KERNEL | (userMode?E_PAGE_USER:0) )) {
