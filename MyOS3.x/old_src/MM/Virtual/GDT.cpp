...
ATOMIC64< SegmentDescriptor > GDT::GDTable[ MAX_GDT_ENTRIES ];
...
GDT::initialize()		// INITSECTION
{
    ...
    SegmentDescriptor code0( E_EXECUTABLE | E_EXECUTEREAD, 0, 0xFFBFF, E_DPL0 ); // at 0xffc00000 page tables start
    SegmentDescriptor data0( E_WRITABLE, 0, 0xFFFFF, E_DPL0 );
    SegmentDescriptor code3( E_EXECUTABLE | E_EXECUTEREAD, 0, 
            ((u32) &_START_OF_KERNEL_)>>12, E_DPL3 ); // @todo not kernel in range?
    
    // @todo currently iVirtualMemory does not allocate elsewhere...
    SegmentDescriptor data3( E_WRITABLE, 0, 0xFF800, E_DPL3 );
