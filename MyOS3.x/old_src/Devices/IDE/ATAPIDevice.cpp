...
{
   return ::operator new<MyOS::Drivers::IDE::IDEDriver,true>( s );   
}

inline void
IORequest::operator delete( void* r )
{
   ::operator delete<MyOS::Drivers::IDE::IDEDriver>( r, sizeof(IORequest) );   
}*/
...
ATAPIDevice::onReadDone( IORequest& rq )
{
   ...
   u16* page = (u16*) IDEDriver::getInstance().iVirtualMemory->allocPage( rq.getAddr() );
   ...
   if ((count&IOCompletionHandler::E_KEEPBUFFER)==0) {
      IDEDriver::getInstance().iVirtualMemory->freePage( page );
   }
