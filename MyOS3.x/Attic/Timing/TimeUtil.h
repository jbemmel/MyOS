...
public:
   static void initIPS( u32 measuredIPS ) INITSECTION;
	...
	inline static u32 now( ETimeUnit unit ) ;
   ...
   template <ETimeUnit unit>
   inline static u64 convertTime( u32 time );
...
template <ETimeUnit unit>
inline u64 TimeUtil::convertTime( u32 time )
{
...
{
	TSC::timestamp end = TSC::read() + convertTime<unit>( time );
	while ( TSC::read() < end );
}

inline u32 TimeUtil::now( ETimeUnit unit )
{
   // this calls udivdi :(
