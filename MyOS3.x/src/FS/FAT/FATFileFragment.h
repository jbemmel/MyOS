...
class FATFileFragment : public MM::Paging::DemandPagedEntity {
   ...
   public: virtual physadr_t fetchNow() throw (MM::Exceptions::PagingException);
   ...
   public:
      ...
      inline FATFileFragment() { memset_aligned(blocklist,0,sizeof(blocklist)); }

      /**
       * XX - Correction factor for BOOT + FAT*2 + ROOT
       * This can be calculated, for now fixed at 31
       */      
      inline void setBlock( u8 n, u16 v ) { blocklist[n] = v + 31; }

      inline void setDevice( IBlockDevice& d ) { device = &d; }
