...
6/9/2007    Tried to be smart using delta between top heap event and new event to be added, 
            but this scheme fails for more than 2 events. Now using 64-bit target RDTSC value
            
            Could use scheduled tick >> 10 (since 1024 instructions more or less accuracy is not
            achievable anyway), then with 32 bits one could handle 2^42 / 10^7 (Bochs ips)
            = 7330 minutes. So after 5 days the wrap around would disturb the system...
            
            Easier to accept the "performance penalty" (not on 64-bit systems..)
*/
 ...
 * 
 * Invariant: foreach HeapItem i in heap h: h.isRoot(i) || h.parent(i).priority <= i.priority
 */
...
public:
	...
	inline T* inspect( size_t n ) const {
	    return (T*) items[n+1];
	}
