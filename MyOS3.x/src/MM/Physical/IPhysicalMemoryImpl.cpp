...
bool
IPhysicalMemoryImpl::init( Context::IContext& context, NVPAIR params[] )   // INITSECTION
{
   // here? Why not..
   ...
   size_t kernelpages = lastPageIndex - firstPageIndex;	
   // DPRINTK( "\n#Kernelpages: %u", kernelpages );
...
//           ALLOCATOR::getPageCount(), &pooldir, target );
...
IPhysicalMemoryImpl::freeInitPages()
{
   ...
   u32 l = (u32) &_COMPRESSED_START_LINEAR_;  // linker variable
   const u32 end = (u32) &_START_OF_CODE_;
   PageDirectory& dir = PageDirectory::getDir( (linadr_t) l );
   for (; l < end; l += _4KB )
   {            
      // clear it!
       memset( (void*) l, 0, _4KB );    // memset_aligned
       pages.free( dir[ getPDTIndex( (linadr_t) l) ].unmap(), 0 );
   }
...
// virtual 
Core::IScriptable* 
IPhysicalMemoryImpl::getScriptable() const throw()
{
    static class Impl : public Core::IScriptable {
        
        const IPhysicalMemoryImpl &intf;
        
    public:
        inline Impl( const IPhysicalMemoryImpl &i ) 
        ...
        {
            DPRINTK( "\nIPhysicalMemoryImpl::Script:%s", call );
...
// virtual 
pteflags_t 
IPhysicalMemoryImpl::getAttributes( linadr_t ofAddr ) const
{
...
// virtual
void
IPhysicalMemoryImpl::clearMapping( linadr_t dst, size_t pagecount )
{
