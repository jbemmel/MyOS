...
if (!impl2.init(context,params)) return E_MYOS_E_FAIL;

if (context.add(impl1)!=E_MYOS_SUCCESS) return E_MYOS_E_REGFAILED;

if (context.add(impl2)!=E_MYOS_SUCCESS) return E_MYOS_E_REGFAILED;
...
context.remove( impl2 );

impl1.deinit(context);

impl2.deinit(context);

return E_MYOS_SUCCESS;
}

myos_result_t MMComponent::queryInterface( 
