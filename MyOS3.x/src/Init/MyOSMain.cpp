...
/*
static IContext& getBootContext( Core::IContainer& core )   // INITSECTION not allowed?
{
    static BootContext bootcontext( core ); // INITSECTION
...
/*
static MyOSCoreContainer& getCore()     // @todo fails when clearing INIT pages (btc)
{
...
void MyOSMain()
{
   ...
   //          All subsequent components are initialized in the new context
   {
      ...
      DPRINTK( "\nEndless loop, result was %x...NMI=%b", r, cmos.isNMIEnabled() );
