...
bool 
IBIOSImpl::init( Context::IContext& context, NVPAIR params[] ) /* INITSECTION */
{
   DPRINTK( "\nBIOS init" );
   ...
   // XXX this breaks null pointer detection!
   mem->setAttributes( 0, 1, E_PAGE_PRESENT | E_PAGE_READWRITE | E_PAGE_USER );

   // (E)IP must also be < _1MB, and it should be 16bit code
   // @todo This is not correct, cannot manage memory like this! Should alloc below 1Mb
   u32 realCode = ((u32)&VM86Code) & (_1MB-1);
   mem->copyMapping( (linadr_t) realCode, (linadr_t) &VM86Code, 1, 
           E_PAGE_PRESENT | E_PAGE_READONLY | E_PAGE_USER );  // too much...
...
void 
IBIOSImpl::deinit( Context::IContext& context )
{
   IInterruptHandling* ih = BIOSComponent::getInstance().iInterruptHandling;
...
// virtual 
linadr_t 
IBIOSImpl::vm86map( linadr_t mem, size_t bytes )
{
    IPhysicalMemory* ipm = BIOSComponent::getInstance().iPhysicalMemory;
    u32 realMem = ((u32) mem & 0xFF000);    // strip page offset too
    ipm->copyMapping( (linadr_t) realMem, mem, DIV_ROUND_UP(bytes,IPhysicalMemory::PAGE_SIZE),
            E_PAGE_PRESENT | E_PAGE_READWRITE | E_PAGE_USER );
    
    return (linadr_t) ((u32) mem & 0xFFFFF);    // max. 20 bits
}

// virtual 
void 
IBIOSImpl::vm86unmap( linadr_t vm86mem, size_t bytes )
{
    IPhysicalMemory* ipm = BIOSComponent::getInstance().iPhysicalMemory;
    ipm->clearMapping( vm86mem, DIV_ROUND_UP(bytes,IPhysicalMemory::PAGE_SIZE) );
}
...
{
   DPRINTK( "\nvm86int[%X] regs.ax=%x regs.es=%x regs.di=%x", i, regs.ax, regs.es, regs.di );
  
   u32* v86stack = new (THREAD) u32[ 256 ];    // allocate v86 stack
   DPRINTK( "\nAllocated 4KB V86 stack at %x", v86stack );
   
   // alias stack to under 1Mb, need only be done once per thread...
   // SS:SP is used to address it
   linadr_t realStackSeg = this->vm86map( v86stack, 256 * sizeof(u32) );
   ...
   this->vm86unmap( realStackSeg, 256 * sizeof(u32) );
   ::operator delete( v86stack, 256 * sizeof(u32), THREAD );
   return E_MYOS_SUCCESS;
}

void  // static, ASM name "VM86MakeCall"
IBIOSImpl::makeCall( u8 intNumber, VM86REGS& regs, u32* v86stack /* in/out */ )
{
   ...
   // save place for current ESP, to be restored in INT3Handler (line 275)
   u32* cur_esp = --vm86sp;
   ...
   *--vm86sp = ( ((u32*) 0)[ intNumber ] );
   ...
   // input registers for POPA in VM86 code (line 253)
...
#define PUSH(x) ASMVOLATILE( "pushl %0" :  : "r"(x) : "esp" );
   ...
   // return address for 'ret' in INT3Handler 
   // PUSH( &&afterIret );  // XXX does not work! optimized away
   ASMVOLATILE( "pushl $afterIret" :  :  : "esp" );
   
   // Now, fill in esp
   *cur_esp = (u32) Processor::ESP();
...
"INT3Handler:                 \n"
"_INT3Handler:                \n"
"PUSH    %ss                  \n"   // restore DS, ES   XXX Dangerous, SP0 stack overwrites!
"POP     %ds                  \n"
"PUSH    %ss                  \n"
"POP     %es                  \n"
"PUSH    %eax                 \n"   // save value
...
"RET                          \n"   // return to "afterIret" on line 165, pushed in line 145
);
