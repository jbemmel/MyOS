...
{
public:
	inline IOPort8() throw() {}

	// Support syntax like IOPort8<n> p = x;
	inline IOPort8( u8 b ) { if (mode!=E_RO) outb(port,b); }

	inline operator u8() const volatile        { return inb(port); }
	inline void operator =( u8 b ) volatile    { outb(port,b); }
	inline void out2( u32 lo_hi ) volatile     { _out2(port,lo_hi); }
	inline void readString( u8* data, size_t bytes ) volatile const { insb(port,data,bytes); }
	inline void writeString( u8* data, size_t bytes ) volatile { outsb(port,data,bytes); }
};
...
public:
    inline operator u8() const volatile                         { return inb(port); }
    inline void readString( u8* data, size_t bytes ) volatile   { insb(port,data,bytes); }
};
...
{
public:
	inline operator u16() const volatile        { return inw(port); }	
	inline void operator =( u16 w )	volatile	{ outw(port,w); }
	inline void readString( u16* data, size_t bytes ) volatile const  { insw(port,data,bytes); }
	inline void writeString( u16* data, size_t bytes ) volatile { outsw(port,data,bytes); }
};
