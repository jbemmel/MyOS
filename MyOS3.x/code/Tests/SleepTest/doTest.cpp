...
/* test TLS, need GCC 4.3.0 or higher for this
__thread int p;
__thread int pi = 1;
   ...
   */
   ...
   static class Sleeper : public IRunnable {
      virtual int run( NVPAIR params[] ) USERSECTION {
         ...
         for (int t = id==0? 10000 : 15000; t < 1 * 1000 * 1000; t<<=1) {
            for (int n=0; n<1; ++n) {
                u32 start = TimeUtil::now( us );
                // DPRINTP( "\n[%d] Start sleep..", start ); 
   ...
   Thread& t = TestComponent::getInstance().getMT().createThread( sleeper, subparams, 1/*, MM::E_DPL3*/ );
