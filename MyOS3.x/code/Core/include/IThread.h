...
public:
   ...
   /**
    * Sends the given signal and optional data to this IThread
    * @param code - signal code (TODO define enum)
    * @param data - optional data
    */
   virtual void signal( u32 code, void* data = 0 ) = 0;

   /**
    * Forces this thread to exit
    *
    * @note MUST NOT be called on self (current thread)
    */
   inline void kill() {
       this->signal( 0xdeaddead, 0 );
   }
...
protected:
   ...
   virtual void exit( int exitcode ) NORETURN = 0;
...
public:
	...
	static inline void do_exit( int exitcode ) NORETURN;
};
...
#include "MT/Thread.h"     // need sizeof(Thread) !
...
inline void IThread::do_exit( int exitcode ) // NORETURN
{
