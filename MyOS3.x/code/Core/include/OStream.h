...
private:
	// Input methods
    virtual EXPORT myos_result_t read( buffer& toBuffer, flags_t options ) { return E_MYOS_E_NOINTERFACE; };
    virtual EXPORT const buffer getBuffer() const { return buffer(0,0); }

    // (for now) unsupported methods
    virtual EXPORT bool setDelimiter( char delim ) { return false; }

protected:
    inline ~OStream() throw() {}
...
inline OStream& operator<<( OStream& o, const UUID& uuid )
{
   o.putBuffer( buffer( (u8*) uuid.asString(),sizeof(UUID)) );
   return o;
}
