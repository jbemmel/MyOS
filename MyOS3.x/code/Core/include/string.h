...
#include "asm.h"     // ASMVOLATILE
#include "types.h"

namespace MyOS {

#ifndef __GNUC__
#include "D:\Program Files\Microsoft Visual Studio\VC98\Include\string.h"
#else
...
/// Optimized strchr, searches per 4 bytes
static inline const char* strchr2(const char * str, int c)
{
	#define __MASK(c) ((c+1) | ((c+1)<<8) | ((c+1)<<16) | ((c+1)<<24))

	// First make sure str is 4 byte aligned
	while ((u32)str&0x3) {
		if (*str==c) return str;
	...
	do {
        u32 dword = *((u32*)str);
        if ((dword-0x01010101) & 0x80808080) { 				// end of string
            break;
        } else if ((dword-__MASK(c)) & 0x80808080) {		// check for c
            break;
        }
        str += 4;
    } while (1);
...
// GCC-provided, GCC can optimize strlen( constant string ), but version 3 needs external code
...
//
// XXX There's some bug in here, it fails on occasion! perhaps clobbers? 
//
    register int __res;
    ASMVOLATILE (
        "addl %0,%2\n\t"
...
static inline void * __memcpy(void * to, const void * from, size_t n)
{
    ...
    ASMVOLATILE(
            ...
            : "memory");
    return (to);
  // return __builtin_memcpy( to, from, n ); causes linker failure, undefined references to "memcpy" ??
}
...
// For GCC 3.4.0 this no longer works
