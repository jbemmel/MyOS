...
public:
     ...
     */
    inline VERSION getImplementationVersion() const
    {
        return version;
    }
    ...
    template<typename TO> inline TO& castToExcept() const throw(CastException)
    {
        TO* t = castTo<TO>();
        if ( t != 0 ) return *t;
        throw CastException( *this, TO::ID() );
    }
