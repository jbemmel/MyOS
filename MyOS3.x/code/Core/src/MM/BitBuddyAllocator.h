 ...
 *
 * 25/9/2008 Essentially the bitpair scheme can result in a situation where some pages
 *           are free but cannot be allocated until the buddy is freed&merged too.
 *           Worst case state is when every odd (or even) page is allocated. Question
 *           is how often this occurs (since the savings are 128 x 4K pages for 4G
 *           in terms of pages not used for bitmaps)
 *
 *
 *  Alternative: Use 4 trees representing 1Gb memory each, use demand paging for the bits
 *        Don't allocate bits for the top X orders (i.e. introduce a maximum allocatable
 *        block < 1Gb). Use fixed 0xffb00000-0xffbfffff range, ensuring that in particular
 *        the order 0 bits are cache aligned (32/64 bytes)
 *
 *  TODO: Make it SMP safe, currently concurrent allocs for different orders can result
 *        in inconsistent allocations
 *
 *  TODO: Do not represent bits for high orders (no need to allocate 4G in one go...)
 *        e.g. start at order 12 for 2^(32-12) = 1MB chunks max. Array index then becomes
 *        -= 2^12 (saving 4K worth of bit memory)
 */
 ...
 * @param O0_BITS: Log2 of the bits in the smallest allocatable unit, 12 for pages of 2^12=4KB
 *
 *
 * @see http://en.wikipedia.org/wiki/Buddy_memory_allocation
 */
template <u8 MAX_ORDER=32, u8 O0_BITS=12>
class BitBuddyAllocator {
   ...
   // MRU cache, this should be L1-cache aligned (32 byte)
   ...
   u32 *bits;   // size == (1 << (MAX_ORDER-O0_BITS)) bits

   public:

   /** @return Number of 4K-pages needed to map the bitmap */
   inline static u32 getPageCount() {
   ...
   void init( linadr_t bitsAddress );
