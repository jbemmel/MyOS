...
template <u8 MAX_ORDER,u8 O0_BITS>
BitBuddyAllocator<MAX_ORDER,O0_BITS>::BitBuddyAllocator()
: totalUnits(0), availableUnits(0), allocs(0), frees(0), bits(0)
{
...
template <u8 MAX_ORDER,u8 O0_BITS>
void
BitBuddyAllocator<MAX_ORDER,O0_BITS>::init( linadr_t bitsAddress )
{
   // L1-cache aligned (32 bytes on Pentium/PPro)
   ASSERTIONv( ((u32)array & 0x1F)==0, E_ERROR, (u32)array );   

   this->bits = (u32*) bitsAddress;
   
   // works for both fixed and variable location
   memset( (void*) bits, 0, (1UL << (MAX_ORDER-O0_BITS-3)) );
...
template <u8 MAX_ORDER,u8 O0_BITS>
unit_t
BitBuddyAllocator<MAX_ORDER,O0_BITS>::allocate( order_t order ) 
{
   ++allocs;
...
template <u8 MAX_ORDER,u8 O0_BITS>
void 
BitBuddyAllocator<MAX_ORDER,O0_BITS>::showMRUUnits() {
...
template <u8 MAX_ORDER,u8 O0_BITS>
void
BitBuddyAllocator<MAX_ORDER,O0_BITS>::free( unit_t frame, order_t order ) {
...
template <u8 MAX_ORDER,u8 O0_BITS>
void
BitBuddyAllocator<MAX_ORDER,O0_BITS>::compact() {
   // iterate over all orders but the top one
...
template <u8 MAX_ORDER,u8 O0_BITS>
void
BitBuddyAllocator<MAX_ORDER,O0_BITS>::getMetrics(
