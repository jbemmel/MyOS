...
boot:	jmp short start			;Standard start of boot sector
	...
	incbin "floppy_bootsector.bin", 3, 0x3b
...
;>   eax = 00000000
	...
	mov	al, [boot+BOOT+BB_fats]	      ;Number of FATs
	mul	byte [boot+BOOT+BB_fat]	      ;Times size of FAT
...
found:	
	push	word [es:edi+0xF]	;Starting cluster of file
	mov	di, [boot+BOOT+BB_fat]	;Size of FAT (in sectors)
	mov	ax, [boot+BOOT+BB_res]	;LBN of FAT
	call	read_sectors
...
.11:	and	bh, 0xF		;Mask to just 12 bits
	...
	mov	al, [boot+BOOT+BB_clu]	;Size of each cluster
...
.10:	push	eax		;LBN
	...
	cdq			;edx = 0
	movzx	ebx, byte [boot+BOOT+BB_sec]
