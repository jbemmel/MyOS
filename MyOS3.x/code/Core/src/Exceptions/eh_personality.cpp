...
struct lsda_header_info
{
  ...
  inline void dump() {
      DPRINTK( "\nLSDA_Header: s=%x lps=%x ttype_base=%x ttype=%x actiontab=%x enc=%x site_enc=%x",
              Start, LPStart, ttype_base, TType, action_table, ttype_encoding, call_site_encoding );
  }
};
...
#if 1 // def _GLIBCXX_SJLJ_EXCEPTIONS
...
extern "C" EXPORT _Unwind_Reason_Code
#ifdef __ARM_EABI_UNWINDER__
...
#endif
{
  ...
  foreign_exception = !__is_gxx_exception_class(exception_class);
    ...
    language_specific_data = (const u8 *) _Unwind_GetLanguageSpecificData (context);
...
#if 1 // def _GLIBCXX_HAVE_GETIPINFO
    ...
    else
    {
        ...
        while (1)
        {
            ...
            else if (ar_filter> 0)
            {
                ...
                // JvB: added, dont match if forced unwinding (thread exit)
                if ( (!catch_type && (actions & _UA_FORCE_UNWIND)==0)
                        || (throw_type
