...
__cxa_throw (void *obj, std::type_info *tinfo,
			 void (*dest) (void *))
{
  // DPRINTK( "\n__cxa_throw obj=%x tinfo=%x destructor=%x", obj, tinfo, dest );
  ...
  _Unwind_Reason_Code code = _Unwind_SjLj_RaiseException (&header->unwindHeader);
#else
  _Unwind_RaiseException (&header->unwindHeader);
#endif

  // Some sort of unwinding error.  Note that terminate is a handler.
  DPRINTK( "\n__cxa_throw: unwinding error: %d", code );
...
__cxa_rethrow ()
{
  ...
  // DPRINTK( "__cxa_rethrow terminate()" );
  std::terminate ();
}
