...
public:
   ...
   inline static Process& getCurrent() CONSTFUNC {
       return *cpuState[ getCurrentCPU() ].currentProcess;
   }
    ...
    */
	...
	inline IThread&  getMainThread() 			{ return mainThread; }
   ...
   // if thread == main (or perhaps threadcount==0 is better??)
	...
	// Allocates space for TLS for all threads in this process, and returns the index
	u32 getNextTLSIndex( u32 varSize, u32 align );
	...
	inline void	prepareSwitch( u32* threadESP )
	{
	    ...
	    cpuState[ getCurrentCPU() ].currentProcess = this;
	}
