 ...
 */
   inline u32 ipsToTicks( u32 ips ) const { 
      return FASTDIV( ips, ticks2c8254 ); 
   }

   // Converts IPS to timer ticks, but no larger than maximum timer capacity
   inline u32 ipsToTicksWithMax( u64 ips ) const { 
      // If ips is only slightly larger than maxPeriodInIPS, prevent doing maxPeriodforTimer?
      return ips < (u64) maxPeriodInIPS 
         ? FASTDIV( (u32) ips, ticks2c8254 ) 
