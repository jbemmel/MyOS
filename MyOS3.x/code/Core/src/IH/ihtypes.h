...
enum E_IRQ
{
   ...
   E_IPI_RESCHEDULE     = 0x20, // first user-assignable vector, 0..31 are CPU reserved
...
enum E_INTCODES {
      ...
      E_IPI_FLUSHTLB          = 0x32,
      E_IPI_THREAD_SELECT     = 0x33,

      /// etc.
      E_NULL_POINTER          = E_PAGE_FAULT,   // same ? different ?

      E_APIC_ERROR            = 0xFE,
      E_APIC_SPURIOUS         = 0xFF,   // Linux uses 0xF. Must be in form 0xhF, where h=0..f
};
