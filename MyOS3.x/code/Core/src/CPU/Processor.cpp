...
struct CPUID
{
    ...
    u8 brandid;
    u8 clflush;
    u8 cpuCount;    //< Number of logical CPUs, for HyperThreading
    u8 apic_initid;

    // ECX entirely reserved (0)
    u32 ecx_res;

    // EDX: expand bits, see http://www.sandpile.org/ia32/cpuid.htm
    union
    {
        u32 features;
        struct
        {
            ...
            unsigned PAE_supported : 1; // Page Address Extensions & NX bit 
            ...
            unsigned __res0__ : 1;
            unsigned SEP_supported : 1; //< SysEnter / SysExit
            ...
            unsigned CLFLUSH : 1;
            unsigned __res1__ : 1;
            unsigned DEBUG_TRACE : 1;
            unsigned THERM_CONTROL_MSR : 1;
            unsigned MMX_supported : 1;
                        
            unsigned CR4_OSFXSR : 1;
            unsigned SSE : 1;
            unsigned SSE2 : 1;
            unsigned selfsnoop : 1;
            unsigned hyperThreading : 1;
            unsigned THERM_INTERRUPT_MSR : 1;
            unsigned IA_64 : 1;
            unsigned PBE : 1;   //< Pending Break Event            
        } PACKED;
    };
} PACKED;

/// Source: Intel System Programming Manual Volume 3A, page 71
enum E_CR4_BITS {
    CR4_VME = (1<<0),
    CR4_PVI = (1<<1),
    CR4_TSD = (1<<2),
    CR4_DE  = (1<<3),
    CR4_PSE = (1<<4),
    CR4_PAE = (1<<5),   //< Physical Address Extension, enables 36-bit addresses
    CR4_MSE = (1<<6),
    CR4_PGE = (1<<7),
...
// see http://www.intel.com/software/products/documentation/vlin/mergedprojects/analyzer_ec/
//       mergedprojects/reference_olh/mergedprojects/instructions/instruct32_hh/vc46.htm
...
bool Processor::enableLocalAPIC()
{
    ...
    if (strncmp(vendor, "GenuineIntel", 12)==0)
    {
        ...
        if (cpu.APIC_enabled)
        {
            DPRINTK("\nIntel CPU with APIC already enabled; HTT=%d lCPUs=%d", 
                  cpu.hyperThreading, cpu.cpuCount );
            return true;
        }
...
;    if ((eax & 0x800)==0)
    ...
    // (eax & (1<<8) == 0x100 ) indicates whether this CPU is the BSP (bootstrap processor)
...
void
Processor::enableNiceFeatures() // TODO: see http://www.ussg.iu.edu/hypermail/linux/kernel/9910.3/0336.html

{ 
    // TODO AMD K5 uses APIC bit to indicate PGE support...
    // XX should check if CPUID supported...      
    CPUID cpu;
    ...
    /* TODO Enable execute-protection for stacks and other data sections
     * Requires a 4th level of page table structures, bit 63 is execute-disable
    if (cpu.PAE_supported) {
        setCR4( CR4() | CR4_PAE );
    } else {
        DPRINTK( "\nAlas, PAE not supported by CPU (features=%x)", cpu.features );
    }
    */
}
