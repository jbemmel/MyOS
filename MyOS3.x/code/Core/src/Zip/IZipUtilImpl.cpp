...
// const uuid_t IZipUtil::ID = "afedc4f2-5f5a-11d6-8272-0010a708e02d";
...
bool 
IZipUtilImpl::init( Context::IContext& context, NVPAIR params[] ) /* INITSECTION */
{
...
void 
IZipUtilImpl::deinit( Context::IContext& context )
{
...
IZipUtilImpl::allocate( size_t s ) const {
   return (u8*) ZipComponent::getInstance().allocator.allocate( s );
}

inline void
IZipUtilImpl::free( u8* mem, size_t s ) const {
    ZipComponent::getInstance().allocator.deallocate( mem, s );
}
