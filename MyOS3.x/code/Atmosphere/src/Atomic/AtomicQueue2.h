...
#ifndef MYATOMICQUEUE2_H
#define MYATOMICQUEUE2_H
...
// the interface
   inline AtomicQueue2() throw() : head(&dummy), tail(&dummy), size(0) {}
   // inline ~AtomicQueue() throw() {}

    inline void enqueue( Node* item )  {
    ...
    */
    inline bool contains( Node* item ) const {
      Node *n = head;
      do {
