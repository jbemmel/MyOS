...
void*
ENTRY::operator new( size_t ) {
    return allocate( sizeof(ENTRY) );
}

void 
ENTRY::operator delete( void *p ) throw() {
...
AdrResolver::handleARPPacket( EthernetPacket& arp )
{
		...
		)
	{
	    ...
	    DPRINTK( "\nARP request(%x): not for us(%x), ignoring...", 
	            targetIP.host(), endpoint.getLocalAddress().host() );
	}
}
