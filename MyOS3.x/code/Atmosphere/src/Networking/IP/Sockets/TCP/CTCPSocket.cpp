...
#include "ITiming.h"
#include "ITimer.h"
#include "CPU/TSC.h"  	// for randomness, TODO: Something else...
...
CTCPSocket::CTCPSocket( IIPEndpoint &ep, ITCPSocketClient *cl )
    ...
    , timer( TCPService::getInstance().iTiming->createTimer( *this ) )
{
	this->init();
}
...
void 
CTCPSocket::operator delete( void* p ) throw()
{
...
CTCPSocket::send( const buffer& data, sendflags_t flags )
{
	...
	if (p) {
		...
		s32 result;		
		if ( (result = state->send( *this, tcph, data.getSize() )) > 0 ) {
			...
			DPRINTK( "\n#4f#Not success but: %x state=%s", result, state->getName() );	
		} // 0 means data was enqueued for delayed transmission
...
// virtual
void
CTCPSocket::onTimer( ITimer& ) throw ()
{
	state->onTimer( *this );
}
...
CTCPSocket::sendFIN( bool finReceived )
{
	...
	if (sendTCPPacket( packet->getData<TCPHeader>().init( TCP_FIN | TCP_ACK ), 
									0, finReceived, 0 )) {
	    
	    // this must be executed at most once! 1 byte for FIN flag
...
CTCPSocket::receiveACK( TCPHeader &p )
{
	...
	if (inWindow(una,maxSeqSent,ack))  {
		...
		// ignore duplicate
		DPRINTK( "\nrxACK: Ign. duplicate ACK #%x una=%x", ack, una );
	} else {	// not yet sent
		DPRINTK( "Got future ACK #%x expected #%x", ack, una );
...
CTCPSocket::receiveData( TCPHeader &segment, size_t ippayload )
{
	...
	if ((tcppayload>0) || segment.isFinish() ) {
		...
		if (tcppayload>0) {
			if (client) {
				DPRINTK( "..client::onRecv size=%u", tcppayload );
				...
				if (delayed) {				
					if (sendTCPPacket( delayed->init(flags), delayedPayload, segment.isFinish(), 0 )) {					    
					    maxSeqSent += delayedPayload;
					}
					delayed = 0;
				} else {
					if (sendTCPPacket( segment.init(flags), tcppayload, segment.isFinish(), E_SEND_PACKET_REUSED)) {
					    maxSeqSent += tcppayload;
					}
				}				
