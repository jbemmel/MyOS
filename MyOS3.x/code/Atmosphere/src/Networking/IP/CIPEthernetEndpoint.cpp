...
CIPEthernetEndpoint::onPacket( Networking::Packet& packet, u32 flags )
{
	// TODO: (IP?) processing in a separate thread / work item? this is IRQ!
	
	// reinterpret as ethernet packet
	EthernetPacket& p = packet.removeHeader<EthernetPacket>();	
	
	DPRINTK( "\n%s:Received packet @%x size=%d protocol=%x",
	        device.getId(), &p, packet.getRxSize(), (u16) p.getProtocol() );
	...
	case ETH_P_IP:
    {
      ...
      CIPEndpoint::receive( ip, packet.getRxSize() - sizeof(EthernetPacket) );	// TODO: Pass flags?
      return;
   }
...
// virtual 
Networking::Packet& 
CIPEthernetEndpoint::allocatePacket( size_t bytes ) throw (MM::OutOfMemoryException)
{
    return * new Networking::Packet( new u8[bytes], bytes );
}

// virtual 
Networking::Packet& 
CIPEthernetEndpoint::allocateCopyCRCPacket( u8* dmabuf, size_t bytes ) 
    throw (MM::OutOfMemoryException)
{    
    Networking::Packet &p = allocatePacket( bytes );
    p.append( dmabuf, bytes );         // TODO: CRC calculation
    return p;
}

// virtual 
void 
CIPEthernetEndpoint::onPacketSent( Networking::Packet& packet )
{
    arp.onPacketSent( packet ); // TODO: too many pass arounds
}
