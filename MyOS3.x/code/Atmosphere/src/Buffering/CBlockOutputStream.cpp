...
CBlockOutputStream::printf( const char* format, ... )
{
   ...
   // Do this in two phases, to get right-sized blocks
   u8 formatBuf[ IOStream::PRINTF_MAX_FORMAT_LENGTH ];
   const buffer b(formatBuf,sizeof(formatBuf));
   BufWriter formatter( b );
   s32 len = formatter.va_snprintf( format, pvar );
   va_end(pvar);
   
   // Do it in two pieces if needed
   size_t space = writer.getSpace();
   if ( len > (s32) space ) {
       bytesWritten += writer.putBuffer( b, space );
       flush();
       len -= space;
   } else space = 0;
   bytesWritten += writer.putBuffer( buffer( &formatBuf[space], len ), len );
...
CBlockOutputStream::putBuffer( const buffer& buf )
{
   ...
   if (size_t bytes = writer.getBytesWritten()) {
      done = min( buf.getSize(), (blocksize-bytes) );
