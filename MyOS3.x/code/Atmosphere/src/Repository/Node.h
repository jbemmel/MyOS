...
public:
   inline NodeIMPL( const myos_name_t& n, VERSION v, IInterface* i=0 ) throw()
	    : name(n), version(v), myInterface(i) {}

   /// Like the above, but considers only l characters of name
   inline NodeIMPL( const myos_name_t& n, size_t l, VERSION v, IInterface* i=0 ) throw()
	    : name(n), version(v), myInterface(i) {}


	// required for treap: default constructor, operator <
	inline NodeIMPL() throw() : name(), version(VERSION::ANY()), myInterface(0) {}

	inline NodeIMPL( const NodeIMPL& n ) throw() 
