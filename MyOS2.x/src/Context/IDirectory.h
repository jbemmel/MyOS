...
protected:
   ...
   IDirectory( Core::IComponent& c, VERSION v )
      : IInterface(c,v) {}

    static const char* const iXML;
	#define IDirectory_UUID "94625155-52fc-48ad-88a8-bf40d3dfa396"
   
public:
   inline static const char* const getXML() { return iXML; }
   inline static myos_name_t uuid() { return myos_name_t(IDirectory_UUID,36); }
...
enum E_ACCEPTFLAGS {
   ...
   A_RECURSIVE       = 0x2,   // Perform same call on subdirectories, if any
};
...
virtual const myos_name_t getName() const { 
   return myos_name_t("IDirectory");
}   

/// Returns the name of this directory, not the name of the interface
virtual const myos_name_t& getDirName() const = 0;
...
addNode( const myos_name_t& name, IInterface& i, u32 flags=E_NONE ) = 0;
 ...
 */
virtual IInterface*
removeNode( const myos_name_t& name, VERSION version = VERSION::ANY() ) = 0;
...
/// Finds a node in this directory
/**
 * @param fullPath   :  Path to the node (may include subdirectories)
 * @param version    :  <optional> version of the node, use VERSION::ANY() for dont care
 * @param result     : [out] result of search
 * @return part of fullPath that was not resolved
 */
virtual myos_name_t
findNode( const myos_name_t& fullPath, VERSION version, IInterface*& result ) const = 0;
...
/// @see IInterface#get
// virtual myos_result_t
//   get( myos_call_t call, NVPAIR parameters[], IO::OStream& output ) const = 0;
