...
#define LOCK { SimpleLock __l; SimpleLock::PROTECTED region(__l);
#define FREE }
#else
...
public:
   ...
   virtual const UUID&  getUUID() const { return * (UUID*)( &iXML[35] ); }

   virtual myos_result_t get( const char* const call, NVPAIR parameters[], IO::OStream& output ) const;
	
	// IDirectory
   virtual const myos_name_t& getDirName() const { return dirname; }	
