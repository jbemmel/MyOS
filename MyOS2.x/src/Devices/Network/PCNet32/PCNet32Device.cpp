...
#include "Drivers/Network/PCNet32/PCNet32Driver.h"
...
#define PHYSM PCNet32Driver::getInstance().iPhysicalMemory
...
PCNet32Device::initialize( IPCISupport& p ) // INITSECTION
{
	...
	dmadata = (DMADATA*) VIRTM->allocate( 4 );
	dmaPhysicalAddress = PHYSM->getPhysicalAddress(dmadata);
...
PCNet32Device::deinitialize()
{
	...
	PCNet32Driver::getInstance().getDM()->getPCISupport().releaseDevice( device );
}
...
PCNet32Device::handleRX()
{
   ...
   while ( (status = dmadata->rxRing[entry].status ) >= 0 && --maxwork)   // use OWN as sign bit
   {
      ...
      if (status & ANYERROR) {
         ...
         rxclient->onPacket( 
         	/// subtract 4+14 bytes for CRC&header
         	buffer( (u8*) desc.userspace, desc.packetSize-4-EthernetPacket::HEADERSIZE ),
            getMatchBits(status)
         );
#ifdef STATISTICS
         ++packetsReceived;
...
PCNet32Device::handleTX(u32 csr0)
{
   ...
   for (u32 maxwork=MAXIRQWORK;maxwork>0; --maxwork, ++dirtyTX)
   {
      ...
      if (status < 0) break;      // not TX'ed yet
		...
		if (data.sender) {
	      	buffer pkt( data.payload, desc.packetSize - sizeof(EthernetPacket) );
	      	if ((status & ERROR)==0) {
				data.sender->onPacketSent( pkt, data.act );      		
			} else {
				data.sender->onPacketAborted( pkt, data.act );
			}
			data.sender = 0;	// avoid further callbacks, XX need more !
		}
      }
	}
...
// virtual 
myos_result_t 
PCNet32Device::open( IPacketReceiver& rx, u32 mode )
{
   switch (status)
   {
	case OPENED: 
	...
	const buffer& buf, u32 flags, IPacketSender* onDone, void* act )
{
   ...
   if (desc.status & OWN) {
	...
	int octets = buf.getSize() + sizeof(EthernetPacket);

	/// Set local address
	EthernetPacket *p = ((EthernetPacket*) buf.getData()) - 1;
	p->setLocalAddress( localAddress );

	desc.misc 	 = CLEAR;
	desc.txBuffer = PHYSM->getPhysicalAddress(&p);

	// 2-complement packetsize
	desc.status = START_TX | ((-octets) & 0x0000FFFF) | (onDone ? TXDONE_INT : 0);
