...
: IEthernetDevice( c, VERSION(1,0) ), rxclient(0) /* was likely already 0 */
, status(E_UNDEFINED)
{
...
// virtual 
myos_result_t 
DP8390D::open( IPacketReceiver& rx, u32 mode )
{
	DPRINTK( "\nDP8390D::open" );
	if (rxclient==0) {
		rxclient = &rx;
...
DP8390D::init( /* params ? */ ) // from DP8390D.pdf page 29
{
	...
	commandRegister = STOP | ABORT_DMA | PS0;
	dataConfig = DMA_WORD | OPERATIONAL | FIFO_TH_8;
	remoteCount0 = 6;	// to read MAC address
	remoteCount1 = 0;
	...
	for (int b=0; b<6; ++b) {
		localAddress[b] = dataport_u8;	// read bytes, not words
	}
	...
	// register IRQ handler, Bochs hardcoded to IRQ9 (TODO: Postpone until open() ? )
	NE2000Driver::getInstance().iInterruptHandling->setIRQHandler( E_IRQ9, *this, E_NONE );
 ...
 */
myos_result_t
DP8390D::sendPacket( const buffer& p, u32 flags, IPacketSender* onDone, void* act )
{
	u32 octets = p.getSize();
	EthernetPacket* pkt = (EthernetPacket*) p.getData();
   ...
   // XX start at txTriggered%bufcount
	...
	for (u8 b=0; b<E_TX_BUFFERS; ++b) {
		...
		if (job->bytes == 0) {
	        ...
	        job->notify = onDone;
	        job->act 	= act;
	        this->blockOutput( (u16*) pkt, octets, job->page );

			if (!txing) {
            	job->bytes = -octets; // mark it processed
            	this->triggerTx( octets, job->page );
         	} else DPRINTK( "postponed" );	// else handleTX() will take care of it
         	result = true;
			break;
      	}
   }
...
void
DP8390D::onInterrupt() // see 8390.c
{
   inInterrupt = true;

   commandRegister = ABORT_DMA | PS0;  // for IRQ status, JVB: add 'START' here?
   u32 status;
   while ((status = isr) != 0) {
      ...
      // switch back to page 0 and START; handleXX routines should do this?
      commandRegister = START | ABORT_DMA | PS0;
   }
...
DP8390D::handleRX()
{
	...
	buffer packet(0,0);	// invalid, filled in when ok
	u32 packetFlags = 0;
	...
	if (rxpage!=thisframe) {
		...
		commandRegister = START | ABORT_DMA | PS0;
		remoteCount0 = sizeof(header);
		remoteCount1 = 0;
		remoteStart0 = 0;    // header starts on page boundary
		remoteStart1 = thisframe;
		commandRegister = START | REMOTE_READ | PS0;

		dataport.readString( (u16*) &header, sizeof(header) );
		...
		// ACK interrupt? Why did I put this here?? this one is not in ALLIRQ
		// isr = RDMA_COMPLETE;
		...
		if ((header.status & 0xF) == E_RXOK 
			...
			blockInput( (u16*) rxCurPacket, framesize, curOffset + sizeof(header) );
			++rxCount;

			packet = buffer( rxCurPacket, framesize-sizeof(EthernetPacket) );
			packetFlags = (header.status & E_MULTICAST) 
				? MULTICAST_MATCH 
				: PHYSICAL_MATCH;
		...
		// update boundary ptr
		boundaryPtr = header.nextptr-1;
	} else {
		BUG( "rxpage==thisframe??" );
	}

	// ACK RX IRQ
	isr = RX_OK | RX_ERROR;
	
	// call client (cannot be null), this may cause out-of-order rx?
	/* EXSAFE_CALLBACK */
	// XX TODO: rxclient should not be null
	if (packet.isValid() && rxclient) rxclient->onPacket( packet, packetFlags );
}
