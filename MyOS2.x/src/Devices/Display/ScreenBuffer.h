...
#include "mem.h"        // memset_aligned with caution!
...
public:
   static ScreenBuffer& getInstance() { return *instance; }
   ...
   public:
      ...
      Cursor( const Cursor& other ) : row(other.row), colom(other.colom) {}
      ...
      bool setPositionRelative( s32 offset ) {
   ...
   private:
      ...
      Cursor( u8 r, u8 c ) : row(r), colom(c) {}
   ...
   inline void clear() {
      memset_aligned( BASE, 0x0F200F20, sizeof(SCREEN) );
      setCursor(0,0);
   }
...
private:
   ...
   ScreenBuffer( void* screenbase ) INITSECTION;
