...
FloppyFSM::init() // INITSECTION
{
   ...
   FloppyDriver::getInstance().iTiming->initTimer( timer, *this );
...
FloppyFSM::addRequest( u32 lbnStart, u8 count, u8 seriesCount, IOCompletionHandler& h ) 
{
   ...
   case S_DELAY_MOTOROFF:
      DPRINTK( "\nCancel timer" );
      FloppyDriver::getInstance().iTiming->cancelTimer( timer );
   ...
   case S_IDLE:
      ...
      curRq = rq;	
      // wait 500 ms
      FloppyDriver::getInstance().iTiming->startTimer( timer, 500 * 1000 );
      break;
...
release:
         ...
         if (curRq) {
			...
			// onTimer_schedule( 3 * 1000 * 1000 );   // 3 seconds
			FloppyDriver::getInstance().iTiming->startTimer( timer, 3 * 1000 * 1000 );
			return;
         }           
      }         
