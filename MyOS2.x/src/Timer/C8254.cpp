...
C8254::init( InterruptHandling::IInterruptContext& irqhandler ) // INITSECTION;
{
   ...
   ih->setIRQHandler( E_IRQ0, irqhandler, E_NONE );		// 17/1/05: changed, disabled IRQs
   ih->enableIRQ( E_IRQ0 );   
}
...
// virtual 
void 
C8254::fireAfterCPUCycles( u32 cpuTicks )
{
   // DPRINTP( "\nfireAfterCpuTicks=%u converted=%d", cpuTicks, ipsToTicksWithMax(cpuTicks) );
