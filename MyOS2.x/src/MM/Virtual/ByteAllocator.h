/**
 * NOTE: Perhaps I should redesign this class to take a reference to a
 * IVirtualMemory interface for allocation. Right now it is tied to 
 * the implementation in MMComponent
 * 
	...
	struct PAGELINK {
      ...
      // dont use constructor, in case '0' is returned when 'nothrow' is set
		inline PAGELINK* init( PAGELINK *p )
		{
         next = 0;
         prev = p;
         bytesfreed.setNonAtomic(sizeof(PAGELINK));
			if (p) p->next = this;
         return this;
		}     
...
public:
   inline explicit ByteAllocator() : currentBlock(0), memory(0) {}

   // It is not always possible to call this constructor
   // If not, call 'init' instead
   inline ByteAllocator( IVirtualMemory& memif, u32 blockorder ) 
   ...
   inline size_t maxBlockSize() { return (1<<blockOrder) - sizeof(PAGELINK); }
     ...
     */
    void* allocate( size_t BLOCKSIZE ) throw (OutOfMemoryException);
    void deallocate( linadr_t block, size_t BLOCKSIZE );

   // Only used by Exception, rare case
   void* allocateNoThrow( size_t BLOCKSIZE ) throw ();
...
class Allocator : protected ByteAllocator
{
#ifdef DEBUG
   size_t allocated;
#endif 
...
public:
   ...
   void* allocateAutoSize( size_t BLOCKSIZE ) 
   ...
   void* allocateAutoSizeNoThrow( size_t BLOCKSIZE ) 
      throw ()
   {
      BLOCKSIZE += sizeof(size_t);
   ...
   void freeAutoSize( linadr_t mem ) {
