...
namespace MyOS { namespace MM {
   ...
   using InterruptHandling::IInterruptHandling;    // install pagefault
...
// custom
virtual linadr_t allocate( size_t order ) throw (OutOfMemoryException);
...
virtual linadr_t allocPage( physadr_t& mapping ) throw (OutOfMemoryException);
virtual linadr_t allocPageNoThrow( physadr_t& mapping );
virtual void freePage( linadr_t vmem );


virtual linadr_t allocateStack( size_t order ) throw (OutOfMemoryException); 
