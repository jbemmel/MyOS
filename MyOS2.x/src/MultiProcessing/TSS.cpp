...
#include "MultiThreading/IThread.h"    // sizeof(Thread)
#include "MultiProcessing/MPComponent.h"

extern "C" void* tss_eip;
...
:  TSS_backlink(0),
   EIP( &tss_eip ), EFLAGS( /*Processor::FLAG_INT*/ 0 ),
...
{
   // State: o.a. LDT==0
   // processes start with interrupts disabled, 'sti' will enable them again
   // as soon as the main thread runs
...
TSS::initOnce()   // INITSECTION
{
   ...
   // Create a single TSS descriptor for all TSS in the GDT (and store it) -> Scheduler must update during taskswitch
...
void
TSS::initialize( linadr_t pbt, Thread &mainthread )
{
   CR3 = MPComponent::getInstance().iPhysicalMemory->getPhysicalAddress( pbt );
   ESP = (void*) (&mainthread); // ->esp not yet set! changes when started, problem?

   // setup stack 0 for Vm86->PM calls
   stacks[0].SS  = SS;
   stacks[0].ESP = (void*) ((u32)ESP - 1024);   // small...
