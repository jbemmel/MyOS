...
template <size_t SIZE>
inline typename AtomicBitArray<SIZE>::index_t
AtomicBitArray<SIZE>::getIndex()
{
    // first make sure there is at least 1
    if (--free<=0) {
        ++free;		// restore it !
        // DPRINTK( "\ngetIndex->FULL, free restored to %d", (u32)free );
        return E_FULL;
    }

    return AtomicBitArrayImpl::getIndex( data, SIZE-1, hint );
}

// returns the start of a range of sequential indices (max. 32)
template <size_t SIZE>
inline typename AtomicBitArray<SIZE>::index_t
AtomicBitArray<SIZE>::getMulti( s8 amount, u8 align /* =1 */ )
{
...
template <size_t SIZE>
inline void
AtomicBitArray<SIZE>::releaseIndex( index_t i )
{
...
template <u32 SIZE>
inline void
AtomicBitArray<SIZE>::markUsed( index_t i )
{
   ASSERTION( i < SIZE, E_ERROR );
 
   // DPRINTK( "mark:%d free=%d |", i, (u32)free );
   if (BTS( data, i )==false) {
      --free;     // Note: need not be atomic
   } else ASSERTIONv( "\nbit not free!"==0, E_WARNING, i );
}

// for locking access to the bitarray when it is empty
template <u32 SIZE>
inline bool
AtomicBitArray<SIZE>::lockIfFree( )
{
...
template <u32 SIZE>
inline void
AtomicBitArray<SIZE>::release( )       // may only be called after lockIfFree returned true
{
