...
bool doTest( IContext& context, NVPAIR params[] )
{
      ...
      void doCommand() {
         ...
         if (strncmp(buffer,"cls",3)==0) {
            ...
            *s = 0;  // terminate interface name
            IInterface *i;
            if (context.lookup( myos_name_t(buffer,s-buffer), i ) == E_MYOS_SUCCESS) {
               ...
               if (i->get( s+2, params, display->cout() )!=E_MYOS_SUCCESS) {
            ...
            } else DPRINTK( "\nCould not find interface %s", buffer );
   ...
   public:
      KeyboardHandler( IContext& ctxt ) : context(ctxt), cur(0) {
         // TODO: Make display a device
         ctxt.lookup( myos_name_t("abe1beba-0457-4375-8a3e-48809a0d5838"), (IInterface*&) display );
      }
   ...
   IInterface *r;
   if (context.lookup( myos_name_t("/dev/kbd0"), r )==E_MYOS_SUCCESS) {
      IKeyboard* kb = r->castTo<IKeyboard>();
