...
public:
   ...
   explicit buffer( const char* zstr ) : size( strlen(zstr) ), data( (u8*) zstr) {}
   
   buffer( u8* d, size_t s ) : size(s), data(d) {}

   // copy constructor, can be partial
   buffer( const buffer& other, u32 ofs=0 )
        : size(other.size-ofs), data(other.data + ofs) {}

   // copy constructor, partial and smaller
   buffer( const buffer& other, u32 ofs, u32 s )
   ...
   template <typename T>
   T& getDataAs() const { return *((T*)data); }
};
