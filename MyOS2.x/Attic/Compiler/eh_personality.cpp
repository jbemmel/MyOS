...
found_something:
   if (landing_pad == 0) {
      ...
      if ((actions & _UA_FORCE_UNWIND) || exception_class != __gxx_exception_class)
         throw_type = 0;
      else
      ...
      while (1)
      {
         ...
         if (ar_filter == 0)
         {
            ...
            if (throw_type && ! check_exception_spec (
               &info, throw_type, thrown_ptr, ar_filter)) {
               saw_handler = true;
               break;
            }
         }
      ...
      } else found_type = (saw_cleanup ? found_cleanup : found_nothing); 
   }  // (long) if
...
do_something:
  ...
  if (actions & _UA_SEARCH_PHASE) {
      ...
      if (exception_class == __gxx_exception_class) {
          ...
          // JvB: Critical for phase2, used in personality routine
...
install_context:
   ...
   if (handler_switch_value < 0) // JvB: ==0 for cleanup handlers
   {
